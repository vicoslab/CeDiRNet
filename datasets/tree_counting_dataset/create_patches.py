...
def _calc_patch_inclusion(patch_xy, patch_size, gt_centers, B=5):
    ...
    #patch_inclusion = patch_inclusion.sum(axis=1)
...
if __name__ == "__main__":
    ...
    assert len(sys.argv), "Error: provide path to tree dataset as first argument to program" 

    TREE_DATASET_DIR = sys.argv[1]

    FILES = [(os.path.join(TREE_DATASET_DIR,'Acacia/12Months_Crop.jpg'),os.path.join(TREE_DATASET_DIR,'Acacia/12Months.csv'),),
             (os.path.join(TREE_DATASET_DIR,'Acacia/6Months_Crop.jpg'),os.path.join(TREE_DATASET_DIR,'Acacia/06Months.csv')),
             (os.path.join(TREE_DATASET_DIR,'Oilpalm/oilpalm-insight-2.jpg'),os.path.join(TREE_DATASET_DIR,'Oilpalm/oilpalm_center.txt'))]

    output_dir = os.path.join(TREE_DATASET_DIR,'/croped_dataset_512x512')
