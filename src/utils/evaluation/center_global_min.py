...
class CenterGlobalMinimizationEval:
    def __init__(self, exp_name='', score_thr=0, tau_thr=30, center_ap_eval=None, display_best_threshold=False):
    ...
    def add_image_prediction(self, im_name, im_index, im_shape, predictions, predictions_score,
        ...
        if self.center_ap_eval is not None:
            self.center_ap_eval.add_image_prediction(predictions, predictions_score, gt_instances, gt_centers_dict, gt_difficult)
        ...
        if return_matched_gt_idx:
            return gt_missed, pred_missed, pred_gt_match, pred_gt_match_idx
        else:
            return gt_missed, pred_missed, pred_gt_match

    def calc_and_display_final_metrics(self, print_result=True, plot_result=True, save_dir=None, **kwargs):
    ...
    def split_samples_into_groups(predictions, gt_centers, distance_thr, grid_count=(8, 8)):
        ...
        return [(sel_idx[sel_idx < len(predictions)],  # prediction ids
                ...
                for sel_idx in clustered_selections if sum(sel_idx < len(predictions)) > 0 or sum(sel_idx >= len(predictions)) > 0]