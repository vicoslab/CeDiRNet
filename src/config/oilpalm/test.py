...
model_dir = os.path.join(OUTPUT_DIR,'{args[dataset][name]}', 'fold=XYZ',
...
args = dict(
    ...
    eval=dict(
        ...
        #     tau_thr=[5,20,40] # 5 == used in paper "Locating Objects Without Bounding Boxes" by Ribera et al.
        # ),
        centers_global_minimization_for_patch_merge=dict(
            display_best_threshold=False,
            tau_thr=[30],  # 40 == used in paper "Counting trees with point-wise supervised segmentation network" by Tong et al.
            merge_threshold_px=60, # 40 == used in paper "Counting trees with point-wise supervised segmentation network" by Tong et al.
            save_raw_results=False,
            only_save_raw_results=False
        ),
    ),
    cross_validation_folds=[0, 1, 2],
    ...
    save_dir=os.path.join(model_dir.replace('fold=XYZ',''),'{args[dataset][kwargs][type]}_results{args[eval_epoch]}-{args[center_checkpoint_name]}--thr{args[center_model][kwargs][local_max_thr]}'),#'border=mirror(64,64,64,64)+max_dist=30px'), #'border=edge(64,64,64,64)'), #
    ...
    dataset={
        ...
        'kwargs': {
            ...
            'type': 'test',
            'fold': 0, 'num_folds': 3,
            'fold_split_axis_rotate': 90, 'fold_split_axis': 'i', 'PATCH_SIZE': 512,
            'fixed_bbox_size': 20,
            'resize_factor': 1.0,
