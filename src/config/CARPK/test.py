...
args = dict(
    ...
    save_prefix='',
    save_dir=os.path.join(model_dir,'{args[dataset][kwargs][type]}_results{args[eval_epoch]}-{args[center_checkpoint_name]}--thr{args[center_model][kwargs][local_max_thr]}', 'border=mirror(64,64,64,64)+max_dist=30px'),
    ...
    # transform any detection found in padding region back to original image based on type of border replication used (edge or mirror/symmetric)
    transform_padding_region_detections=True,
    transform_padding_region_detections_max_border_dist=30,     # for mirror/symmetric padding
